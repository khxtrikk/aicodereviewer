...
export default function ReposPage() {
    ...
    const handleConnect = () => {
        const reposToConnect = availableRepos.filter((r) => selectedRepos.has(r.githubId)).map((r) => ({
            githubId: r.githubId,
            name: r.name,
            fullName: r.fullName,
            private: r.private,
        ...
        <div className="space-y-8">
            <div className="flex items-start justify-between gap-4">
                <div>
                    ...
                    <p className="text-muted-foreground mt-1">Select repositories to connect to your account.</p>
                </div>
                <Button
                    onClick={() => {
                                    ...
                                    {filteredAvailableRepos.length === 0 ? (
                                        <div className="py-12 text-center">
                                            <p className="text-sm text-muted-foreground">
                                                {" "}
                                                No repositories match your search.</p>
                                        </div>
                                    ) : (
            ...
            <div className="space-y-4">
                <div className="flex items-center justify-between">
                    <h2 className="text-sm font-medium text-muted-foreground uppercase tracking-wider">Connected Repositories</h2>
                    ...
                    <Card>
                        <CardContent className="py-16 text-center">
                            ...
                            <p className="text-sm text-muted-foreground mt-1 max-w-sm mx-auto">
                                Connnect your Github repositories to start getting AI-powered
    ...
    return (
        <Card className="group hover:border-primary/30 transition-all hover:shadow-sm">
            <CardContent className="p-5">
                <div className="flex items-center gap-3 justify-between">
                    <Link href={`/repos/${repo.id}`} className="flex-1 min-w-0">
                        <div className="flex items-start gap-3">
                            <div className={cn(
                                ...
                                repo.private ? "bg-emerald-500/10 group-hover:bg-emerald-500/15" : "bg-gray-500/10 group-hover:bg-gray-500/15"
                            ...
                            <div className="min-w-0">
                                <span className="font-medium block truncate group-hover:text-primary transition-colors">{repo.fullName}</span>
                    ...
                    <AlertDialog>
                        <AlertDialogTrigger
                            ...
                            className="shrink-0 cursor-pointer text-muted-foreground opacity-0 transition-opacity hover:bg-destructive/10 hover:text-destructive group-hover:opacity-100"
                        >
                        ...
                        <AlertDialogContent>
                            <AlertDialogHeader>
                                ...
                                <AlertDialogDescription>Are you sure you want to disconnect {" "}"
                            ...
                            <AlertDialogFooter>
                                ...
                                <AlertDialogAction 
                                    ...
                                    className="bg-destructive text-destructive-foreground hover:bg-destructive/90 cursor-pointer"> Confirm</AlertDialogAction>
                            </AlertDialogFooter>
                        </AlertDialogContent>
                    </AlertDialog>
                ...
                <span className="text-xs text-muted-foreground">Connected {formatDate(repo.createdAt)}</span>
                <Link href={`/repos/${repo.id}`}>
                <Button
                    variant={"ghost"}
                    size={"sm"}
                    className="h-7 text-xs gap-1.5 -mr-2 cursor-pointer"
                >
                    View PRs
...
function RepoSelectItem({ repo, selected, onToggle }: {
    repo: GithubRepo;
    selected: boolean;
    onToggle: () => void;
}) {
    ...
    return (
        <label className={cn(
            "flex items-center gap-4 cursor-pointer px-6 py-4 transition-colors",
            ...
            <div className="felx items-center gap-4 shrink-0">
                {repo.stars > 0 && (
                    <span className="flex items-center gap-1 text-xs text-muted-foreground">
                        <Star className="size-3" />
                        <span className="tabular-nums"> {repo.stars}</span></span>
                )}
                {repo.language && (
